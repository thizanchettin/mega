...
def loader(spark: SparkSession, src_df: DataFrame, catalog: str, schema: str, table: str) -> bool:
    ...
    try:
        ...
        src_df.write.mode("overwrite").saveAsTable(f"{catalog}.{schema}.{table}")
        logger.info(f"Data loaded successfully into table {catalog}.{schema}.{table}")
        return True
    except Exception as e:
        logger.error(f"Error loading data into table {catalog}.{schema}.{table}: {e}")
        return False